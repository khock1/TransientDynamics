function [Astates, Mstates, col] = f_discrER_reps_BW_reproducefigures(es, timesteps)
%clearvars -except r reps S es ee uui;
%es=[0];
%es=0:0.05:1;
showfigs=[5 29 28];% F1 15 10 20 25 F2 20 F3 5 20 % 1 4 5 8 9 10 11 13 15 16 17 %1 4 5 9 10 15 16
...
cyctimes=0;%find(genseqnc('p', timesteps, 3));%find(genseqnc('n', 50, [1 0.1429]));%[find(genseqnc('p', 20, 7)) find(genseqnc('n', 30, [1 0.1429]))];%find(genseqnc('p', timesteps, 7));
%cyctimes(cyctimes > (timesteps-5))=[];
%cyctimes=unique([cyctimes 15 17]);
%cyctimes(cyctimes > 25)=[];
blchtimes=0;%find(genseqnc('p', timesteps, 3));%find(genseqnc('p', timesteps, 4));
%blchtimes(blchtimes > (timesteps-5))=[];
%blchtimes=unique([blchtimes 5 7]);
graztimes=0;%1:50;
grazrecovery=210;
conntimes=0;%1:timesteps;%find(genseqnc('p', timesteps, 2));
...
htmAAT=zeros(length(unique(spc(:,1))));
PRM=zeros(length(unique(spc(:,1))),length(es));
CC=zeros(length(unique(spc(:,1))),length(es));
SPD=zeros(length(unique(spc(:,1))),length(es));
%set(gca,'Color','k');
xco=0:0.01:1;
yco=0:0.01:1;
...
%     end
    ...
    S1=solve(dy1==0, As);
    S2=solve(dy2==0, As);
    plh=fplot(S1(1),[0 1]);set(plh,'color',[0.5 0.5 0.5],'linewidth',2);%plh=plot(0:0.1:1,zeros(1,11));set(plh,'color','b','linewidth',2);plh=fplot(S1(1),[0 1]);set(plh,'color','b','linewidth',2);
    %plh=fplot(S2(1),[0 1]);set(plh,'color','green','linewidth',2);
    plh=plot(zeros(1,11),0:0.1:1);set(plh,'color','k','linewidth',2);%plh=plot(zeros(1,11),0:0.1:1);set(plh,'color','green','linewidth',2);plh=fplot(S2(1),[0 1]);set(plh,'color','green','linewidth',2);
...
%multicolour phase plot, just coral change
if ismember(29,showfigs)
    figure;hold;
    for iii = 1:size(spc,1)
        if mod(iii,10)==0
            thissst=iii;
            ...
            ch=zeros((length(xx)),1);
            for c=1:(length(xx))
                ch(c)=abs(Astates(c,thissst)-Astates(c+1,thissst));
            end
            zz = zeros(size(xx));
            col=transpose(ch);
            surf([xx;xx],[yy;yy],[zz;zz],[col;col],...
                'facecol','no',...
                'edgecol','interp',...
                'linew',1);
...
if ismember(26,showfigs)
    ...
    for ii = 1:size(spc,1)
        ...
        htmAAT(x(1,:)==spc(ii,1),y(1,:)==spc(ii,2))=aat;
    end
...
if ismember(28,showfigs)
    ...
    S1=solve(dy1==0, As);
    S2=solve(dy2==0, As);
    plh=fplot(S1(1),[0 1]);set(plh,'color','k','linewidth',2);%plh=plot(0:0.1:1,zeros(1,11));set(plh,'color','b','linewidth',2);plh=fplot(S1(1),[0 1]);set(plh,'color','b','linewidth',2);
    %plh=fplot(S2(1),[0 1]);set(plh,'color','green','linewidth',2);
    plh=plot(zeros(1,11),0:0.1:1);set(plh,'color',[0.5 0.5 0.5],'linewidth',2);%plh=plot(zeros(1,11),0:0.1:1);set(plh,'color','green','linewidth',2);plh=fplot(S2(1),[0 1]);set(plh,'color','green','linewidth',2);
    plh=fplot(S1(2),[0 1]);set(plh,'color',[0.5 0.5 0.5],'linewidth',2);
    plh=fplot(S2(2),[0 1]);set(plh,'color','k','linewidth',2);
    
    for jj=1:size(unstabeq,1)
        scatter(unstabeq(jj,1),unstabeq(jj,2),100,'d','filled','k');
    end
    for jj=1:size(stabeq,1)
        scatter(stabeq(jj,1),stabeq(jj,2),100,'o','filled','k');%if stable
    end
    ylabel('Coral cover (%)');
    xlabel('Macroalgal cover (%) ');
    axis([0 axlim 0 axlim]);
